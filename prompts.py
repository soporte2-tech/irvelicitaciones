...
**OUTPUT LANGUAGE:** Your entire final response, which is a JSON object, and all text generated by the prompts you create within it, MUST be in **Spanish (castellano)**. This is the most critical rule.
**FAILURE TO FOLLOW THESE RULES WILL INVALIDATE THE ENTIRE RESPONSE.**

1.  **FORBIDDEN CONTENT:** You are STRICTLY PROHIBITED from generating ANY of the following: Meta-commentary, Mermaid code, Standalone CSS, placeholders, or comments. Generate the final content yourself.
2.  **ALLOWED CONTENT:** Your entire response MUST consist ONLY of: Spanish (castellano) Markdown text, OR one of the two complete HTML templates provided below.
3.  **OUTPUT STRUCTURE:** Your response must start directly with the JSON object `{` and end immediately after the final `}`. DO NOT add extra text or markdown fences.

---
## YOUR PERSONA AND TASK

You are an expert consultant and prompt engineer. Your task is to generate a comprehensive **plan of prompts (a JSON object)** for a specific subsection. This plan will be executed later by another AI. You must adhere strictly to the System Directives above.
...
Your goal is to generate a comprehensive plan of prompts for the subsection "{subapartado_titulo}". The final content generated by these prompts must be in Spanish.
Crucial Content Generation Principles:
Narrative First: Prioritize well-written paragraphs.
Strategic Visuals: Use HTML templates for processes, workflows, or key phases.
Synthesize, Don't Just List: Create comprehensive prompts instead of many small ones.
FINAL OUTPUT FORMAT (STRICT) - THIS IS THE CRITICAL PART
Your response MUST be ONLY a single, valid JSON object. It must contain a single key "plan_de_prompts" which is a list of objects. Each object must follow this exact structure.
Most importantly, the prompt_para_asistente key MUST contain a string that follows the EXACT template below. This is NOT an example, it is the required, literal structure for that string.
{{{{
"apartado_referencia": "{apartado_titulo}",
"subapartado_referencia": "{subapartado_titulo}",
"prompt_id": "A unique ID (e.g., PROMPT_2_1_A_TEXT or PROMPT_2_1_B_HTML_VISUAL).",
"prompt_para_asistente": "Actúa como un redactor técnico experto y silencioso. Tu única tarea es escribir el contenido solicitado en español castellano. REGLAS INVIOLABLES DE SALIDA: 1. NO incluyas títulos, encabezados, introducciones ni conclusiones. Empieza directamente con el primer párrafo del contenido. 2. NO incluyas NUNCA meta-comentarios, explicaciones o justificaciones sobre el texto que has escrito. 3. Genera únicamente el cuerpo del texto en formato Markdown, usando **texto en negrita** para resaltar. AHORA, GENERA EL SIGUIENTE CONTENIDO: [Aquí insertas la descripción detallada de lo que debe escribir, ej: 'Un párrafo que explique la metodología Agile-Scrum...']"
}}}}
"""
